# Wiki Index

## Orientation
- [[home]] — Obsidian landing page for navigation, priorities, and current research lanes.
- [[overview]] — Top-level map of the cybersecurity research wiki.
- [[scope]] — Scope, boundaries, and what belongs in this repo.
- [[taxonomy]] — Preferred organizing axes, tags, and terminology decisions.

## Topics
- [[topics/authorized-security-research]] — Legal and ethical frame for this wiki.
- [[topics/penetration-testing]] — What penetration testing is, where it fits, and how it differs from adjacent practices.
- [[topics/penetration-testing-distributions]] — Purpose-built operating-system environments for authorized assessment work.
- [[topics/red-teaming]] — Goal-based adversary emulation and defensive validation.
- [[topics/gray-hat]] — Notes on the informal gray/grey-hat label and why it is not a primary taxonomy.
- [[topics/red-hat-label]] — Notes on the informal red-hat label and why it is not used as the main organizing concept.
- [[topics/threat-modeling]] — Threat-modeling as a planning and prioritization discipline.
- [[topics/attack-mapping]] — Using ATT&CK-style mapping to connect research findings to adversary behavior.
- [[topics/lab-safety-and-rules-of-engagement]] — Safe lab practice, authorization, and rules of engagement.

## Frameworks
- [[frameworks/nist-sp-800-115]] — NIST guide to information security testing and assessment.
- [[frameworks/owasp-wstg]] — OWASP Web Security Testing Guide.
- [[frameworks/mitre-attack]] — ATT&CK as an adversary-behavior knowledge base and mapping framework.

## Sources
- [[sources/kali-linux-website-corpus-2026-04-05]] — Corpus-level summary of Kali Linux and related OffSec web pages staged externally on 2026-04-05.

## Entities
- [[entities/kali-linux]] — Security-focused Linux distribution used as an assessment environment.
- [[entities/offsec]] — Organization appearing alongside Kali-related material in the current corpus.

## Analyses
- `wiki/analyses/` stores durable answers, comparisons, timelines, and synthesis pages created from research questions.

## Templates
- [[templates/source-summary-template]] — Template for source pages.
- [[templates/topic-template]] — Template for topic and concept pages.
- [[templates/analysis-template]] — Template for durable research outputs.
